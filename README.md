...
Hotovo: jeden non-root Docker kontejner, healthcheck, persistentní SQLite/asset volume, PWA, sessions/CSRF/login rate-limit, živé SSE aktualizace, přesný RCON klient, zpětně kompatibilní telemetry V3, minutové i dlouhodobé výrobní grafy položek a kapalin, planetární Operations, výzkum s ETA, přehled platforem, energetiky, logistických sítí a pojmenovaných sond, porovnání až čtyř prototypů, výrobní cíle s herním oznámením, úkoly zakládané a dokončované přímo ve hře, blueprint inspector, 44 úspěchů, skutečné Factorio ikony s CZ/EN názvy, automatický report směny, activity/log tailer, drag & drop úkoly s editací, termíny, filtry, připnutím a archivem, sdílené vzkazy s reakcemi a potvrzením přečtení, rozšířené profily, bezpečné informační RCON nástroje, stažení aktuálního modu, mobilní burger menu a responzivní světlé i tmavé UI.
