...
function OperationsShell({ title, description, capability, state, children }: { title: string; description: string; capability?: OperationsResponse['capabilities'][number]; state: ReturnType<typeof useOperationsData>; children: (operations: OperationsResponse) => ReactNode }) {
  const { operations, error } = state;
  return <section className="content operations-view"><PageHeader eyebrow="SPACE AGE TELEMETRY" title={title} description={description} actions={operations && <span className="telemetry-version">LIVE · {timeAgo(operations.generatedAt)}</span>} />{error && <p className="error banner">{error}</p>}{!operations ? <div className="page-loading"><p>Načítám telemetry…</p></div> : capability && !operations.capabilities.includes(capability) ? <div className="telemetry-upgrade"><strong>Telemetry tuto část zatím neposílá.</strong><p>Po nasazení aktuálního modu se stránka sama naplní. Ostatní části aplikace zůstávají funkční.</p></div> : children(operations)}</section>;
}
...
function PlanetsView() {
  ...
  return <OperationsShell title="Planety" description="Přehled planetárních povrchů, jejich provozu a přiřazených operátorů." capability="surface-flow" state={state}>{(operations) => {
...
function ProductionView() {
  ...
  return <section className="content">
    <PageHeader eyebrow={textFor(locale, 'productionEyebrow')} title={kind === 'item' ? textFor(locale, 'productionTitle') : 'Kapaliny'} description={kind === 'item' ? textFor(locale, 'productionDescription') : 'Výroba a spotřeba kapalin napříč všemi planetami a platformami.'} actions={<div className="production-header-actions"><div className="flow-kind-picker"><button type="button" className={kind === 'item' ? 'active' : ''} onClick={() => { setKind('item'); setSelectedItems([]); }}>Položky</button><button type="button" className={kind === 'fluid' ? 'active' : ''} onClick={() => { setKind('fluid'); setSelectedItems([]); }}>Kapaliny</button></div><div className="range-picker" aria-label={textFor(locale, 'timeRange')}>{(['1m', '15m', '1h', '6h', '24h', '7d', '30d', '1y'] as ProductionRange[]).map((value) => <button type="button" className={range === value ? 'active' : ''} key={value} onClick={() => setRange(value)}>{value}</button>)}</div></div>} />
...
class ViewErrorBoundary extends Component<{ view: View; children: ReactNode }, { error: Error | null }> {
  ...
  render() { return this.state.error ? <section className="content"><PageHeader eyebrow="OCHRANA APLIKACE" title="Tahle stránka narazila na neúplná data" description="Zbytek PWA zůstává funkční. Zkuste stránku znovu načíst; chyba je zapsaná v konzoli pro diagnostiku." /><button className="primary" onClick={() => window.location.reload()}>Načíst PWA znovu</button></section> : this.props.children; }
}
