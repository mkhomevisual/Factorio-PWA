...
function ProductionView() {
  ...
  const chartData = useMemo(() => {
    if (chartType !== 'bar' || comparisonData.length <= 80) return comparisonData;
    const stride = Math.ceil(comparisonData.length / 80);
    return comparisonData.filter((_row, index) => index % stride === 0 || index === comparisonData.length - 1);
  }, [chartType, comparisonData]);
  ...
  const colors = ['#ff9f43', '#52d3c2', '#b28cff', '#5aaeff'];
  return <section className="content">
    ...
    <Panel title={textFor(locale, selectedItems.length ? 'comparisonTitle' : 'factoryFlowTitle')} subtitle={textFor(locale, selectedItems.length ? 'comparisonSubtitle' : 'factoryFlowSubtitle')} action={<div className="chart-controls"><div className="chart-mode chart-type" role="group" aria-label={locale === 'cs' ? 'Typ grafu' : 'Chart type'}><button type="button" className={chartType === 'line' ? 'active' : ''} onClick={() => setChartType('line')}>{locale === 'cs' ? 'Křivka' : 'Line'}</button><button type="button" className={chartType === 'bar' ? 'active' : ''} onClick={() => setChartType('bar')}>{locale === 'cs' ? 'Sloupce' : 'Bars'}</button></div><div className="chart-mode" role="group" aria-label={locale === 'cs' ? 'Zobrazený tok' : 'Displayed flow'}><button type="button" className={comparisonMetric === 'both' ? 'active' : ''} onClick={() => setComparisonMetric('both')}>{textFor(locale, 'both')}</button><button type="button" className={comparisonMetric === 'productionRate' ? 'active' : ''} onClick={() => setComparisonMetric('productionRate')}>{textFor(locale, 'production')}</button><button type="button" className={comparisonMetric === 'consumptionRate' ? 'active' : ''} onClick={() => setComparisonMetric('consumptionRate')}>{textFor(locale, 'consumption')}</button></div></div>} className={`chart-panel ${loading ? 'is-refreshing' : ''}`}>
      {loading && production && <span className="chart-refresh-indicator"><Glyph name="refresh" />{textFor(locale, 'refreshing')}</span>}
      {!production ? <div className="loading-inline production-loading">{textFor(locale, 'loadingProduction')}</div> : chartData.length ? <div className="production-chart" aria-busy={loading}><ResponsiveContainer width="100%" height="100%">{chartType === 'line' ? <LineChart data={chartData} margin={{ top: 10, right: 8, left: -18, bottom: 0 }}><CartesianGrid vertical={false} stroke="var(--chart-grid)" /><XAxis dataKey="at" tickFormatter={(value) => new Date(value).toLocaleTimeString(locale, { hour: '2-digit', minute: '2-digit', second: range === '1m' ? '2-digit' : undefined })} minTickGap={35} stroke="var(--text-tertiary)" tickLine={false} axisLine={false} /><YAxis stroke="var(--text-tertiary)" tickLine={false} axisLine={false} tickFormatter={(value) => compactNumber.format(value)} /><Tooltip labelFormatter={(value) => new Date(String(value)).toLocaleString(locale)} contentStyle={{ background: 'var(--surface-raised)', border: '1px solid var(--border-strong)', borderRadius: 12, boxShadow: 'var(--shadow-lg)', backdropFilter: 'blur(18px)' }} />{selectedItems.length ? selectedItems.map((item, index) => <Fragment key={item}>{comparisonMetric !== 'consumptionRate' && <Line type="monotone" dataKey={`item${index}Production`} name={`${labelFor(labels, item)} · ${textFor(locale, 'production')}`} stroke={colors[index]} strokeWidth={3} dot={range === '1m'} activeDot={{ r: 5 }} />}{comparisonMetric !== 'productionRate' && <Line type="monotone" dataKey={`item${index}Consumption`} name={`${labelFor(labels, item)} · ${textFor(locale, 'consumption')}`} stroke={colors[index]} strokeWidth={2.5} strokeDasharray="7 5" strokeOpacity={0.82} dot={range === '1m'} activeDot={{ r: 5 }} />}</Fragment>) : <>{comparisonMetric !== 'consumptionRate' && <Line type="monotone" dataKey="productionRate" name={textFor(locale, 'productionPerMinute')} stroke="var(--production)" strokeWidth={3} dot={range === '1m'} activeDot={{ r: 5 }} />}{comparisonMetric !== 'productionRate' && <Line type="monotone" dataKey="consumptionRate" name={textFor(locale, 'consumptionPerMinute')} stroke="var(--consumption)" strokeWidth={3} dot={range === '1m'} activeDot={{ r: 5 }} />}</>}</LineChart> : <BarChart data={chartData} margin={{ top: 10, right: 8, left: -18, bottom: 0 }} barGap={2}><CartesianGrid vertical={false} stroke="var(--chart-grid)" /><XAxis dataKey="at" tickFormatter={(value) => new Date(value).toLocaleTimeString(locale, { hour: '2-digit', minute: '2-digit', second: range === '1m' ? '2-digit' : undefined })} minTickGap={35} stroke="var(--text-tertiary)" tickLine={false} axisLine={false} /><YAxis stroke="var(--text-tertiary)" tickLine={false} axisLine={false} tickFormatter={(value) => compactNumber.format(value)} /><Tooltip labelFormatter={(value) => new Date(String(value)).toLocaleString(locale)} cursor={{ fill: 'color-mix(in srgb, var(--accent) 7%, transparent)' }} contentStyle={{ background: 'var(--surface-raised)', border: '1px solid var(--border-strong)', borderRadius: 12, boxShadow: 'var(--shadow-lg)', backdropFilter: 'blur(18px)' }} />{selectedItems.length ? selectedItems.map((item, index) => <Fragment key={item}>{comparisonMetric !== 'consumptionRate' && <Bar dataKey={`item${index}Production`} name={`${labelFor(labels, item)} · ${textFor(locale, 'production')}`} fill={colors[index]} fillOpacity={0.94} radius={[4, 4, 0, 0]} maxBarSize={18} />}{comparisonMetric !== 'productionRate' && <Bar dataKey={`item${index}Consumption`} name={`${labelFor(labels, item)} · ${textFor(locale, 'consumption')}`} fill={colors[index]} fillOpacity={0.44} radius={[4, 4, 0, 0]} maxBarSize={18} />}</Fragment>) : <>{comparisonMetric !== 'consumptionRate' && <Bar dataKey="productionRate" name={textFor(locale, 'productionPerMinute')} fill="var(--production)" fillOpacity={0.94} radius={[5, 5, 0, 0]} maxBarSize={24} />}{comparisonMetric !== 'productionRate' && <Bar dataKey="consumptionRate" name={textFor(locale, 'consumptionPerMinute')} fill="var(--consumption)" fillOpacity={0.72} radius={[5, 5, 0, 0]} maxBarSize={24} />}</>}</BarChart>}</ResponsiveContainer></div> : <EmptyState>{locale === 'cs' ? 'Čekám na první telemetry vzorek verze 2.' : 'Waiting for the first telemetry version 2 sample.'}</EmptyState>}
      <div className="chart-legend">{selectedItems.length ? selectedItems.flatMap((item, index) => [comparisonMetric !== 'consumptionRate' && <span key={`${item}-production`}><i style={{ background: colors[index] }} />{labelFor(labels, item)} · {textFor(locale, 'production')}</span>, comparisonMetric !== 'productionRate' && <span key={`${item}-consumption`}><i className={chartType === 'line' ? 'dashed' : 'bar-muted'} style={{ '--line-color': colors[index], background: colors[index] } as CSSProperties} />{labelFor(labels, item)} · {textFor(locale, 'consumption')}</span>]) : <>{comparisonMetric !== 'consumptionRate' && <span><i className="production" />{textFor(locale, 'production')}</span>}{comparisonMetric !== 'productionRate' && <span><i className={`consumption ${chartType === 'bar' ? 'bar-muted' : ''}`} />{textFor(locale, 'consumption')}</span>}</>}</div>
    </Panel>
...
export function App() {
  ...
  useEffect(() => { document.documentElement.dataset.theme = theme; document.querySelector('meta[name="theme-color"]')?.setAttribute('content', theme === 'dark' ? '#070b0d' : '#eef2ef'); }, [theme]);
