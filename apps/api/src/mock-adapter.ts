...
export class MockFactoryAdapter implements FactoryAdapter {
  async getSnapshot(_afterEventId?: string): Promise<FactorySnapshot> {
    ...
    return {
      ...
      sharedFactory: baseCounters.map(([item, produced, consumed], index) => ({ item, produced: produced + elapsedMinutes * (index + 4) * 7, consumed: consumed + elapsedMinutes * (index + 4) * 6 })),
      events: [
